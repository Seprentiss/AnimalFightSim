...
class Cougar:
    ...
    def ClimbTree(self):
        ...
        if self.inTree is False and oppInTree is True:
            treeClimbed = random.choices(["Yes", "No"], weights=(50, 50))
            if treeClimbed[0] == "Yes":
                self.inTree = True
                self.attPT -= 1
                return self.inTree
    ...
    def PlainsRandAttack(self):
        ...
        att = random.choices(attacks, weights=(30, 70), k=1)
        if att[0] == "Bite":
    ...
    def JungleRandAttack(self):
        ...
        if self.inTree is True and oppInTree is True:
            ...
            att = random.choices(attacks, weights=(35, 65), k=1)
            if att[0] == "Bite":
        ...
        if self.inTree is False and self.oppInTree is True:
            ...
            att = random.choices(attacks, weights=(30, 75), k=1)
            if att[0] == "Bite":
                hit = random.choices(['T', 'F'], weights=(15, 85))
                if hit[0] == "T":
                    attPow = 0
                else:
                    attPow = self.bite
